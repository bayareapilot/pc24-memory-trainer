...
/* The panel plates are ~5.6 MB across 63 files. Precaching them in `install`
 * would stall the update the user just tapped, and leaving them to the runtime
 * cache means a plate you have not opened yet is missing in a sim building with
 * no signal. So: warm them after activate, sequentially, ignoring failures. The
 * install stays fast, and the images are there by the time they are needed.
 * The list is generated by build.py, so it can never drift from panel_refs.json.
 */
async function warmPanels() {
  try {
    const res = await fetch('./images/panels/index.json', { cache: 'reload' });
    if (!res.ok) return;
    const names = await res.json();
    const cache = await caches.open(CACHE_NAME);
    for (const name of names) {
      const url = `./images/panels/${name}`;
      if (await cache.match(url, { ignoreSearch: true })) continue;
      try { await cache.add(new Request(url, { cache: 'reload' })); } catch (e) { /* keep going */ }
    }
  } catch (e) { /* offline during activate — the runtime cache still covers it */ }
}
...
self.addEventListener('activate', event => {
  event.waitUntil(
    caches.keys()
      ...
      // Not awaited as part of the critical path — activate must not wait on 5.6 MB.
      .then(() => { warmPanels(); })
  );
});
